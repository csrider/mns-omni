...
#	NOTE: TAKEN FROM IPSPEAKER LOGIC!
#
...
#	3b.Upload AND_Gen2_IPSpeakerConfigurationSchema.xsd as the XML Schema (available at http://www.ipspeaker.com/support20/AND_Gen2_IPSpeakerConfigurationSchema.xsd).
...
# 2016.07	CSR	Fixed LOTS of syntax issues (per AND's analysis of our cfg files), and now supporting firmware v1.5 (the syntax issues were not allowing the sign to update). Details below, per Sean Lantz at AND:
#	- There should be whitespace (carriage return or at least a space) between the "SIPConfig" tag and the first parameter, "SIP_server_addr"
#	- There should be whitespace after each parameter and the next:
#		- Between SIP_server_addr="192.168.1.58" and SIP_port="5060"
#		- Between SIP_port ="5060" and id="716"
#	- There is a permanent stream channel fragment for 239.168.3.9 within the Firmware20 and Firmware21 file definitions.  This must be enclosed/defined within the PermanentStreams tag.  This same stream is repeated below in the proper location, so it looks like it was improperly added to this section.
#	- Several streams are duplicated in the PermanentStreams section, 239.168.3.8, 239.168.3.7, 239.168.3.6, 239.168.3.5, 239.168.3.4 - these duplicates should be removed.
#	- There is a fragment of stream 239.168.3.3 within the other streams that should be cleaned up.
#	- After the closing tag </IPSpeakerConfiguration>, 3 permanent streams (.3, .2, and .1) and the display settings and closing tag are repeated again - these were removed.
...
#else
#NOTE: DUMPISAM COMMAND NOT DEVELOPED YET!
#	ADDRESS=`$SILENTM_HOME/bin/dumpisam -evolution_address_lookup $USERID`
#	if [ "$ADDRESS" != "" ]
#	then
#		#curl "http://$ADDRESS:8080/reboot?password=$AUTHCODE" 2> /dev/null > /dev/null
#		curl "http://$ADDRESS:8080/restart?password=$AUTHCODE" 2> /dev/null > /dev/null
